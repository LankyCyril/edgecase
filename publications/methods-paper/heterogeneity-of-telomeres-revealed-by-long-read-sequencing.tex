...
We have validated these methods using Genome in a Bottle \cite{giab} single-molecule real-time (SMRT) sequencing datasets generated with Pacific Biosciences circular consensus sequencing (PacBio CCS) \cite{pacbio,pacbioccs} and short read Illumina \cite{illumina} datasets.
...
We performed \textit{de novo} repeat discovery in the supported regions for motifs of lengths 4 through 16 and identified motifs in repeat contexts that are statistically enriched in all three datasets.
...
When the entire candidate PacBio CCS sequences were considered, the top enriched motif (TTAGGG) explained 79.9\% of the telomeric component, while all enriched motifs combined explained 87.7\%.
...
%\subsection*{Telomeric heterogeneity is evident in short, linked and long read sequencing of human subjects and cell lines}
%\addcontentsline{toc}{subsection}{Telomeric heterogeneity is evident in short, linked and long read sequencing of human subjects and cell lines}
%\textcolor{WCM}{Catch-all for PacBio CHM, astronaut Illumina and 10X} \\
%\textcolor{WCM}{Motif variation present in all samples including real humans} \\
%\textcolor{WCM}{Diplotypes or even triplotypes in telomeres of CHM, possibly astronauts. Mole diplotype is fine, cite paper that mentioned telo LD.}
%\textcolor{WCM}{Illumina serves as proximal} \\
%\textcolor{WCM}{OR BETTER: 10X serves as haplotype resolution}
...
To target motifs in repeat contexts and to distinguish motifs differing by indels (for example, ACGT and ACGTT), doubled sequences (for example, \textit{k}-mer ACGTACGT for motif ACGT) were counted with \textit{jellyfish} \cite{jellyfish}, and counts of \textit{k}-mers synonymous with respect to circular shifts (for example, ACGTACGT and CGTACGTA) were summed together.
...
The Bonferroni multiple testing correction was applied to the resultant \textit{p}-values, and motifs for which \textit{k}-mers yielded \textit{p}-values below the cutoff of 0.05 were reported.
\\~\\
...
%\subsection*{Proximal Illumina OR BETTER haplotypic 10X in astronauts}
%\addcontentsline{toc}{subsection}{Proximal Illumina OR BETTER haplotypic 10X in astronauts}
